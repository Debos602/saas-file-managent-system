...
const changePassword = async (user: any, payload: any) => {
    ...
    await prisma.user.update({ where: { email: userData.email }, data: { password: hashedPassword } });
...
const resetPassword = async (token: string | null, payload: { email?: string, password: string; }, user?: { email: string; }) => {
    ...
    // Case 2: Authenticated user (resetting while logged in)
    ...
    await prisma.user.update({ where: { email: userEmail }, data: { password } });
};
